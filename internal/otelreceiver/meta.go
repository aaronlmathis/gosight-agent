...
func convertMetaToResource(meta *model.Meta) *resourcepb.Resource {
	...
	// --- OS / Platform / Kernel ---
	add("host.os", meta.OS)
	add("host.os.version", meta.OSVersion)
	add("host.platform", meta.Platform)
	add("host.platform.family", meta.PlatformFamily)
	add("host.platform.version", meta.PlatformVersion)
	add("host.arch", meta.Architecture)
	add("host.kernel.version", meta.KernelVersion)
	add("host.container", meta.VirtualizationSystem)
	add("host.container.role", meta.VirtualizationRole)
	...
	add("k8s.pod.uid", meta.PodUID)
	add("k8s.namespace.name", meta.Namespace)
	add("k8s.namespace.uid", meta.NamespaceUID)
	add("k8s.deployment.name", meta.DeploymentName)
	add("k8s.deployment.kind", meta.OwnerKind)
	add("k8s.cluster.name", meta.ClusterName)
	add("k8s.cluster.uid", meta.ClusterUID)
	add("k8s.node.name", meta.NodeName)
	add("k8s.service_account.name", meta.ServiceAccount)

	// Kubernetes pod/node labels
	for k, v := range meta.PodLabels {
		add("k8s.pod.label."+k, v)
	}
	for k, v := range meta.NodeLabels {
		add("k8s.node.label."+k, v)
	}
	for k, v := range meta.PodAnnotations {
		add("k8s.pod.annotation."+k, v)
	}

	// --- Application / OTel Resource / Service context ---
	add("service.name", meta.ServiceName)
	add("service.namespace", meta.ServiceNamespace)
	add("service.instance.id", meta.ServiceInstanceID)
	add("service.version", meta.ServiceVersion)
	add("telemetry.sdk.name", meta.TelemetrySDKName)
	add("telemetry.sdk.version", meta.TelemetrySDKVersion)
	add("telemetry.sdk.language", meta.TelemetrySDKLanguage)
	add("otel.library.name", meta.InstrumentationLibrary)
	add("otel.library.version", meta.InstrumentationLibVersion)
	...
	addInt("process.id", meta.ProcessID)
	add("process.name", meta.ProcessName)
	add("process.executable", meta.Executable)
	add("process.runtime.name", meta.RuntimeName)
	add("process.runtime.version", meta.RuntimeVersion)

	// --- Networking / Mesh / Security context ---
	add("host.ip", meta.IPAddress)
	add("net.host.ip", meta.PublicIP)
	add("net.host.private_ip", meta.PrivateIP)
	add("net.host.mac", meta.MACAddress)
	add("net.host.interface", meta.NetworkInterface)
	add("mesh.peer.version", meta.MeshPeerVersion)
	addBool("security.tls.enabled", meta.MTLSEnabled)
	add("security.tls.version", meta.TLSVersion)
	add("security.tls.cipher", meta.CipherSuite)
	add("rpc.auth.method", meta.AuthMethod)
	add("enduser.id", meta.User)

	// JWT claims (flattened as "jwt.claims.<key>")
	for k, v := range meta.JWTClaims {
		add("jwt.claims."+k, stringify(v))
	}

	// --- Deployment / CI-CD context ---
	add("deployment.id", meta.DeploymentID)
	add("git.commit", meta.GitCommitHash)
	add("build.time", meta.BuildTimestamp)

	// --- Log‐specific fields ---
	add("app.name", meta.AppName)
	add("app.version", meta.AppVersion)
	add("systemd.unit", meta.Unit)
	add("event.id", meta.EventID)
	add("log.file.path", meta.Path)

	// --- Extra (collector-specific fields) ---
...
// stringify converts interface{} to string for resource attributes.
