...
// OtlpHttpReceiver encapsulates OTLP ingestion receivers for metrics and logs.
...
// NewOtlpHttpReceiver constructs a new OtlpHttpReceiver based on sysCtx.Cfg.OpenTelemetry.
func NewOTelHTTPReceiver(ctx context.Context, cfg *config.Config) (*OtlpHttpReceiver, error) {

	// Create a new router for the OTLP receiver
	router := mux.NewRouter()
	// Create an HTTP server for OTLP ingestion
	httpServer := &http.Server{
		Addr:    cfg.OTLPReceiver.HTTPAddr,
...
func (o *OtlpHttpReceiver) Start() error {
	o.setupRoutes()
	go func() {
		<-o.ctx.Done()
...
func (o *OtlpHttpReceiver) setupRoutes() {
...
func (o *OtlpHttpReceiver) handleMetricIngest(w http.ResponseWriter, r *http.Request) {
...
func (o *OtlpHttpReceiver) handleLogIngest(w http.ResponseWriter, r *http.Request) {
	ct := r.Header.Get("Content-Type")
	if r.Method != http.MethodPost || !strings.HasPrefix(ct, "application/x-protobuf") {
		http.Error(w, "Unsupported method or content type", http.StatusUnsupportedMediaType)
		return
	...
	logEntries := OTLPToLogEntries(&req)
...
func (o *OtlpHttpReceiver) handleTraceIngest(w http.ResponseWriter, r *http.Request) {
	ct := r.Header.Get("Content-Type")
	if r.Method != http.MethodPost || !strings.HasPrefix(ct, "application/x-protobuf") {
		http.Error(w, "Unsupported method or content type", http.StatusUnsupportedMediaType)
		return
...
func (o *OtlpHttpReceiver) Shutdown() error {
	utils.Info("Shutting down OTEL HTTP server...")

	if err := o.httpServer.Shutdown(o.ctx); err != nil {
