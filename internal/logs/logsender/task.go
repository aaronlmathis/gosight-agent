...
// StartWorkerPool launches N workers and processes log batches with retries
...
func (s *LogSender) StartWorkerPool(ctx context.Context, queue <-chan []model.LogEntry, workerCount int) {
	for i := 0; i < workerCount; i++ {
		...
		go func(id int) {
			...
			for {
				...
				if err := s.SendLogs(batch); err != nil {
					utils.Warn("Log worker #%d failed to send batch: %v", id, err)
				}
			}
		}(i + 1)
	}
